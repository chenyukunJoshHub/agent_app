...
Working Memory（单次 LLM Call 的 Context Window）
  ...
  RAG chunk（P2）：每轮基于当前输入重新检索，ephemeral 注入
  Token 预算分配：各区块固定上限 + 弹性历史区
  超限时自动压缩旧消息
```

### 1.3.1 RAG Chunk 注入策略（P2）
...
选择的策略：Ephemeral 注入
  RAG chunk 在 wrap_model_call 里注入请求级 System Prompt
...
每次 Agent turn 完整生命周期：
  ...
  wrap_model_call → LLM 调用前，注入用户画像到请求级 System Prompt
  [ LLM 调用 ]
...
before_agent 触发（整个 turn 只执行一次）
      │
      ▼
从 PostgresStore 读取用户画像
  store.aget(namespace=("episodic", user_id), key="profile")
      │
      ▼
反序列化为 EpisodicData
  {
...
after_agent 触发（整个 turn 结束后执行一次）
  写回 state["memory_ctx"].episodic 到 PostgresStore

🔴 P0（面试前 · 仅需跑通存储链路）：
  Long Memory 在面试演示中感知弱，P0 阶段 MemoryMiddleware 的
  before_agent / after_agent 钩子结构需建好，但写回内容是空操作
  目的：保证钩子挂载正确，不影响 P0 主链路

⚪ P2 第一步（面试后 · 基础版）：
  画像内容开始有变化：interaction_count +1
  写回内容 ≈ 读出内容，验证读写链路通畅
...
用户输入
  ...
  → before_agent：从 PostgresStore 加载用户画像 → state["memory_ctx"]
  → Short Memory 自动恢复（AsyncPostgresSaver 全自动，无需手写）
  → wrap_model_call：注入用户画像到请求级 System Prompt
                     ⚪ P2：注入 RAG chunk（ephemeral，不写回 state）
  → ReAct 推理循环
      → LLM 决策（思考 / 调用工具 / 给出答案）
      → 工具执行（并行）
      → 结果注入 Context，继续循环
  ...
  → AsyncPostgresSaver 自动保存
      只保存 HumanMessage + AIMessage
      RAG chunk 不写入，Short Memory 保持干净
  → after_agent：写回用户画像到 PostgresStore
  → 记录 AgentTrace 日志
```
...
③ RAG chunk（⚪ P2，默认关闭）  < 500 Token   ephemeral，每轮重新检索，不写入 Short Memory
...
create_agent → MemoryMiddleware.before_agent
  → PostgreSQL：store.aget(namespace=("episodic", user_id))
  ← EpisodicData → 写入 state["memory_ctx"]
...
create_agent → MemoryMiddleware.wrap_model_call
  ...
  → 注入用户画像到请求级 System Prompt（ephemeral，不写回 state）
  → ⚪ P2：注入 RAG chunk（同样 ephemeral，不写回 state，不污染 Short Memory）
...
create_agent → PostgreSQL
  → AsyncPostgresSaver.save(HumanMessage + AIMessage)
  → RAG chunk 不写入，Short Memory 只保存纯对话历史

─── ⑥ after_agent（turn 结束，触发 1 次）───────────────────────────
create_agent → MemoryMiddleware.after_agent
  → ⚪ P2 第一步：interaction_count +1
  → ⚪ P2 第二步：规则提炼本轮偏好
  → PostgreSQL：store.aput(namespace=("episodic", user_id), value=EpisodicData)
...
✅ LangChain 内置，配置即用
  ...
  PostgresStore             长期记忆存储层（langgraph.store.postgres）
...
⚡ 胶水代码，少量自行开发
  db/postgres.py            AsyncPostgresSaver + PostgresStore 初始化
...
② Assemble context window
   ...
   · RAG chunk 注入（P2）          MemoryMiddleware.wrap_model_call    🔧 自行开发
...
钩子              触发时机               职责                       归属
──────────────────────────────────────────────────────────────────────────
before_agent     每次 turn 开始，1 次    从 PostgresStore 加载       🔧 MemoryMiddleware
                                        EpisodicData
                                        存入 state["memory_ctx"]
                                        后续 wrap_model_call
                                        直接读缓存，不重复 IO
...
after_agent      每次 turn 结束，1 次    写回 EpisodicData 到         🔧 MemoryMiddleware
                                        PostgresStore
                                        🔴 P0：空操作（钩子结构建好）
                                        ⚪ P2 第一步：+1 interaction_count
                                        ⚪ P2 第二步：关键词规则提炼偏好
                                           示例：消息含"合同""签署"
...
store 表          PostgresStore.setup() 自动建           ✅ LangChain 内置
...
长期记忆存储层           PostgresStore                          ✅ LangChain 内置            ⚪ P2
长期记忆读写逻辑         MemoryManager                          🔧 自行开发                  ⚪ P2
  第一步：interaction_count +1（读写链路跑通）
  第二步：规则提炼（关键词 → 写入偏好字段）
用户画像注入逻辑         MemoryMiddleware.wrap_model_call        🔧 自行开发                  ⚪ P2
RAG chunk 注入          MemoryMiddleware.wrap_model_call         🔧 自行开发                  ⚪ P2
  策略：Ephemeral，每轮重新检索，不写入 Short Memory
  位置：请求级 System Prompt，不写回永久 state
...
│   └── postgres.py                # ⚡ AsyncPostgresSaver + PostgresStore 初始化
├── supabase/
...
│   ├── schemas.py                 # 🔧 EpisodicData / MemoryContext / AgentState
│   └── long_term/
...
后端基础设施
  ...
  ⚡ db/postgres.py（AsyncPostgresSaver + PostgresStore 初始化，自动建表）
...
```
长期记忆（用户画像）
  决策：长期记忆在面试演示中感知弱（需两次会话才可见），面试后按序实现

  第一步：基础版
    🔧 Episodic Memory 写回（interaction_count +1）
    🔧 MemoryMiddleware.before_agent 加载画像 → state["memory_ctx"]
    🔧 MemoryMiddleware.after_agent 写回 PostgresStore
    验收：跑通完整读写链路，画像内容尚无实质变化

  第二步：规则提炼
    🔧 after_agent 中加入简单规则：
        用户提到"茅台""A股" → preferences.domain = "stock"
        用户提到"合同""签署" → preferences.domain = "legal"（E签宝场景）
        用户使用中文         → preferences.language = "zh"
    🔧 wrap_model_call 注入用户画像到请求级 System Prompt
    验收：第二次会话，Agent 回复能体现用户领域偏好
...
RAG 集成（见 § 1.3.1）
  🔧 Semantic Memory（pgvector + RAG）
  🔧 RAG chunk ephemeral 注入（MemoryMiddleware.wrap_model_call）
