...
│  │  │ Slot ②  (删除)活跃技能内容（Active Skill，经 ToolMessage 进历史     │ │   │
│  │  │ Slot ③  动态 Few-shot（P1）                                 │ │   │
│  │  │ Slot ④  RAG 背景知识（P2）                                  │ │   │
...
Slot ⑤ 用户画像注入（Episodic，Ephemeral）
  ...
  原因：LangChain 官方推荐——situational context 注入 messages 末尾，模型对末尾注意力更高；
        system_message 保留给静态行为指令（角色定义、Procedural）
  示例注入文本：
    [用户画像]
...
④ [RAM → LLM] Context Window 最终内容
    ┌───────────────────────────────────────────────────────────────────┐
    │ Slot ①  SystemMessage：角色定义 + Skill Registry 元数据 + 静态 Few-shot │
    │         （纯静态，不含用户画像）                                    │
...
Procedural             Slot ⑥        400 Token   Long Memory(DB)      ❌    每次 LLM 调用前      P2
                       （Ephemeral：注入 SystemMessage，wrap_model_call → request.override(system_message=...)
                         原因：程序性记忆是行为规则，属于 system 静态指令职责）
...
| RAG Retriever           | 检索 + Ephemeral 注入                           | `AgentMiddleware.wrap_model_call` 内检索后 `request.override(messages=...)` 前置于最后 HumanMessage | 🔧 自行开发     | ⚪ P2      |
...
#   Procedural → request.override(system_message=...)
#     程序性记忆是行为规则，属于 system 静态指令职责
...
class MemoryMiddleware(AgentMiddleware):
    async def awrap_model_call(
        ...
        # Episodic：取用户画像（P0），前置注入最后一条 HumanMessage
        item = await request.runtime.store.aget(("profile", user_id), "profile")
        ...
        if episodic_text:
            ...
            if last_human_idx is not None:
                ...
                messages.append(HumanMessage(content=episodic_text))
            request = request.override(messages=messages)  # Ephemeral

        # Procedural（P2）：注入 SystemMessage（行为规则，适合 system 职责）
        # patterns = await request.runtime.store.asearch(
        #     ("procedural", user_id), query=request.state["messages"][-1].content, limit=3
        # )
        # if patterns:
        #     procedural_text = format_procedural(patterns)
        #     existing = request.system_message
        #     new_content = (existing.content if existing else "") + procedural_text
