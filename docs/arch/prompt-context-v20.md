...
>
> 实现状态标注：
> - **已实现（当前代码）**：可直接在仓库运行链路中验证
> - **目标态（未实装）**：架构预留设计，当前代码仅有占位或预算配置
...
│  │  ┌────────────────────────────────────────────────────────────┐ │   │
│  │  │ Slot ①  System Prompt（角色定义）                            │ │   │
│  │  │ Slot ②  Skill Registry 元数据                                │ │   │
│  │  │ Slot ③  Skill Protocol 约束                                  │ │   │
│  │  │ Slot ④  Few-shot（当前静态；动态检索为目标态 P1）              │ │   │
│  │  │ Slot ⑤  RAG 背景知识（目标态 P2）                             │ │   │
│  │  │ Slot ⑥  用户画像（Episodic Memory）                           │ │   │
│  │  │ Slot ⑦  程序性记忆（Procedural，目标态 P2）                   │ │   │
│  │  │ Slot ⑧  工具定义 Schema（框架自动注入）                        │ │   │
│  │  │ Slot ⑨  会话历史（含推理轨迹 + Active Skill ToolMessage + 本轮输入）│ │   │
│  │  │ Slot ⑩  输出格式规范                                          │ │   │
│  │  └────────────────────────────────────────────────────────────┘ │   │
│  └──────────────────────────────────────────────────────────────────┘   │
│           │                                                              │
...
来源分类（决定 Slot 内容从哪里来）：
  ...
  Agent Skills  ：read_file(path=...) 调用 → ToolMessage → Slot ⑨ 历史区  → Skill 文件系统（工具读取，持久化）
  短期记忆      ：会话历史 + ReAct 推理轨迹（Slot ⑨）                      → Short Memory（DB → RAM）
  长期记忆      ：Episodic（Slot ⑥）+ Procedural（Slot ⑦）                → Long Memory（DB → RAM）
  动态检索（目标态，当前未实装）：
                 RAG chunk（Slot ⑤，P2）+ 动态 Few-shot（Slot ④，P1）      → 向量存储（DB → RAM）
...
补充说明：
  ...
  Active Skill 预算             → 配置中保留 token_slot_active_skill（默认 0），
                                   但运行态不做独立 slot，内容计入 Slot ⑨ 历史
...
- Skill 详细内容不进入独立 Slot；通过 `read_file(path=...)` 返回 ToolMessage 进入 Slot ⑨
- 用户画像（[6]）是 Ephemeral，每次 LLM 调用前重新注入，不写入对话历史
- 动态 Few-shot（Slot ④）是目标态能力（当前未实装），通过语义检索获取，与静态 Few-shot 区分
...
└── 可用于输入：24,576 Token
    ...
    ├── Slot ①  System Prompt（角色定义）
    │           2,000 Token   P0
    ├── Slot ②  Skill Registry（运行态可视槽，预算默认 0）
    │           默认 0         当前主要作为可视化与提示跟踪项
    ├── Slot ③  Skill Protocol（运行态可视槽，预算默认 0）
    │           默认 0         当前主要作为可视化与提示跟踪项
    ├── Slot ④  Few-shot（静态已实装；动态检索为目标态）
    │           800 Token    P1，默认 0
    ├── Slot ⑤  RAG 背景知识
    │           2,000 Token   P2，默认 0
    ├── Slot ⑥  用户画像（Episodic）
...
P1 固定占用（Few-shot 激活）：3,700 + 800 = 4,500 Token
P1 弹性历史区：24,576 - 4,500 = 20,076 Token（skill 内容经 ToolMessage 进入历史区）

P2 固定占用（全开）：4,500 + 2,000 + 400 = 6,900 Token
P2 弹性历史区：24,576 - 6,900 = 17,676 Token（压缩触发频率极低）
...
- 当前实现将 Active Skill 作为预算预留项（默认 0）；skill 内容通过 ToolMessage 进入 Slot ⑨
...
当前实现：
  精确计数：tiktoken 库（`app/utils/token.py::count_tokens`，默认启用）
  近似估算：仅作为兜底策略（文档保留概念，不是当前默认实现）
```
...
Memory Injector 职责范围（仅管理 Long Memory → Context Window 的注入）：
  ...
  ✅ Memory Injector 负责：
     · Episodic（用户画像）：Long Memory → Slot ⑥（逻辑归属，Ephemeral 注入 messages）
  ...
  ❌ Memory Injector 不负责（由框架或其他模块处理）：
     ...
     · Active Skill 内容 → read_file(path=...) 工具调用后进入 Slot ⑨ 历史区
     · Slot ④ 动态 Few-shot → Few-shot Retriever（目标态，当前未实装）
     · Slot ⑤ RAG 背景知识 → RAG Retriever（目标态，当前未实装）
...
Slot ⑥ 用户画像注入（Episodic，Ephemeral）
  ...
  时机：turn 开始时从 DB 加载到 RAM（state["memory_ctx"].episodic）
        每次 LLM 调用前从 RAM 读取，前置注入最后一条 HumanMessage（request.override(messages=...)）
  特点：不写入 state["messages"] → 历史永远干净
  注入位置：messages 数组（非 system_message）
  原因：当前实现将动态上下文统一注入 messages，避免在 system 指令层重复堆叠
  示例注入文本：
    [用户画像]
...
Slot ⑦ 程序性记忆注入（Procedural，Ephemeral，P2）
  来源：Long Memory  namespace=("profile", user_id), key="procedural"
...
Slot ⑨ 会话历史（Short Memory，持久化）
  ...
  内容分类：
    ...
    ToolMessage(read_file)     → skill.md 完整内容（与普通工具结果同类型，
                                  但内容是操作手册而非数据，LLM 用于指导后续行为）
  注意：
    · AIMessage 中的 Thought 文字就是 ReAct 推理轨迹，随历史持久化
    · read_file 读取的 Skill ToolMessage 也在此处，session 内后续 turn 持续可见
...
#### ⑦ RAG Retriever（P2，默认关闭，当前未实装）

**职责**：根据用户输入从知识库检索相关背景，Ephemeral 注入 Slot ⑤。

当前状态：代码仅保留预算与接口占位，未接入向量检索执行链路。
...
**职责**：管理 Agent Skills 的注册与加载。初始化时将 Skill Registry 元数据写入 System Prompt（Slot ②），推理时 LLM 通过标准 Function Calling 触发 `read_file(path=...)`，完整 skill 内容以 ToolMessage 形式进入 `state["messages"]`（Slot ⑨ 历史区，持久化）。
...
阶段一：初始化注入（Slot ①，< 100 Token，P1 启用时）
  ...
  示例元数据格式（写入 Slot①）：
    ...
    激活方式：调用工具 read_file(path="<skill file_path>")

阶段二：推理时动态加载（标准 Function Call，P1）

  ⚠️ 关键机制：read_file 是一个普通 @tool 函数，
  走标准 LangGraph Function Calling 流程，不需要 Skill 专用 Middleware。

  完整调用链：
    ① LLM Thought：判断需要 legal-search skill
    ② LLM Action：输出 tool_call → read_file(path="skills/legal-search/SKILL.md")
    ③ Tool Executor：执行 read_file 函数
    ④ 函数内部：从文件系统读取 legal-search.md 内容
  ...
  与普通工具调用的区别：
    ...
    read_file(读取 Skill) → 返回操作指南文本（供 LLM 用来指导后续行为）
```
...
注意：skill ToolMessage 占用 Slot ⑨（历史区）的 Token 预算（~1,500 Token），
      非独立 slot；active_skill 的预算描述是概念上的"为 skill 内容预留"
```
...
**节点一：Compressor 压缩 Prompt**（触发时机：Slot ⑨ 历史区接近上限）

```
调用方   ✅ SummarizationMiddleware（框架内置）
触发条件 默认 trigger=("fraction", 0.75)，keep=("messages", 5)
LLM 调用 在 middleware 内部执行摘要，不需要自定义 summarize_node
...
当前项目状态：
  · 已接入 SummarizationMiddleware（create_summarization_middleware）
  · 尚未自定义压缩 prompt 模板（使用框架默认行为）
  · 前端 compressionEvents 目前仍是占位数组（后续可接入压缩事件 SSE）
```
...
② [DB → RAM] Long Memory 加载（每 turn 一次，不重复访问 DB）
    ...
    │                                                                  │
    │   episodic = UserProfile(           # 用户画像（来自 Long Memory） │
    │     preferences = {                                                 │
    ...
    │   ),                                                             │
    │   procedural = ProceduralMemory(     # 工作流 SOP（P2，默认空）      │
    │     workflows = {                                              │
    │       "合同查询": "1) 调用 contract_status\\n2) 解释待签方与DDL"   │
    │     }                                                          │
    │   )                                                            │
    │ )                                                                │
    └─────────────────────────────────────────────────────────────┘
...
③ [RAM → Ephemeral] 组装工作记忆（本质：把 RAM 中的数据序列化成文本）
    ...
    Slot ①  静态内容（角色定义 + 能力边界 + Skill Registry 元数据 + 静态 Few-shot） ~800 Token
            Slot ① 合计 = 800 Token（< 900 上限）✓

    [MemoryMiddleware → request.override(messages=...)]
    Slot ⑥  用户画像（Episodic，注入最后一条 HumanMessage 前部）   ~100 Token
    ...
    ⚪ P1 [Skill 加载] read_file 是普通 @tool，走标准 Function Calling
           LLM 输出 tool_call → read_file(path="skills/legal-search/SKILL.md")
    ...
    ⚪ P1 [DB → Ephemeral] 动态 Few-shot 检索（默认关闭，当前未实装）
    向量存储 → Embedding 相似度检索 → top-2 示例 → Ephemeral 注入 Slot ④

    ⚪ P2 [DB → Ephemeral] RAG 检索（默认关闭，当前未实装）
    向量存储 → 语义检索 → 相关 chunk → Ephemeral 注入 Slot ⑤

④ [RAM → LLM] Context Window 最终内容
    ┌───────────────────────────────────────────────────────────────────┐
    │ Slot ①-④ 静态区：System + Skill Registry + Skill Protocol + Few-shot │
    │            （纯静态，不含用户画像）                                 │
    │ Slot ⑥  Episodic 前置于最后一条 HumanMessage（Ephemeral）          │
    │         → messages 数组，来自 state["memory_ctx"].episodic          │
    ...
    │         ← 本轮用户输入也在此区（作为最新 HumanMessage）             │
    └───────────────────────────────────────────────────────────────────┘
    ※ Active Skill 内容实际落在 Slot ⑨ 历史区，
      通过 read_file ToolMessage 自然出现，非独立注入位置
    → LLM 调用
...
# 当前项目：通过 MemoryMiddleware.state_schema 扩展 memory_ctx，
# 并在 create_agent(..., context_schema=AgentContext) 传入请求级上下文
...
│  │     "summary": "..."                                                 │  │
│  │   },                                                                 │  │
│  │   "procedural": {                ← 工作流 SOP（P2，默认{}）         │  │
│  │     "workflows": {                                                    │  │
│  │       "合同状态查询": "1) 调用 contract_status\\n2) 解释待签方与DDL"   │  │
│  │     }                                                                │  │
│  │   }                                                                  │  │
│  │ }                                                                    │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
...
class MemoryContext(TypedDict, total=False):
    episodic:    dict          # UserProfile.model_dump() 的输出，用户画像
    procedural:  dict          # ProceduralMemory.model_dump()，含 workflows（P2）
...
#### Episodic Memory（用户画像）→ Slot ⑤（注入 messages，Ephemeral）
...
注入到 messages（前置最后一条 HumanMessage）的文本形态（Ephemeral，不写入历史）：
...
#### Agent Skill（技能定义）→ Slot ② 元数据 + Slot ⑨ ToolMessage（P1）
...
class SkillMetadata(BaseModel):
    ...
    path:        str
    # skill.md 文件路径，供 read_file(path=...) 调用时使用
...
class ProceduralMemory(BaseModel):
    # 当前实现（P0/P1）：单记录结构，和 backend/app/memory/schemas.py 一致
    # namespace=("profile", user_id), key="procedural"
    workflows: dict[str, str]
    # 示例：
    # {
    #   "合同状态查询": "1) 调用 contract_status\\n2) 解释待签方与DDL",
    #   "批量提醒": "1) 先 query_overdue\\n2) HIL 确认\\n3) send_email"
    # }

# ── 当前实现存取方式（与代码一致）────────────────────────────────────────────
# 写入（merge 语义）：
#   existing = await store.aget(("profile", user_id), "procedural")
#   payload = existing.value if existing else {}
#   payload.update(new_data)
#   await store.aput(("profile", user_id), "procedural", payload)
#
# 读取（abefore_agent）：
#   item = await store.aget(("profile", user_id), "procedural")
#   procedural = ProceduralMemory(workflows=item.value.get("workflows", {})) if item else ProceduralMemory()
#
# P2 目标（可选演进）：
#   若需要语义检索 top-k 成功模式，再拆分为多 entry + asearch 方案
```

注入 Slot ⑥ 的文本形态（Ephemeral，P2）：
来源：`store.aget(("profile", user_id), "procedural")` 读取 workflows 后格式化注入
...
#### Few-shot Example（目标态，当前未实装）→ Slot ① 静态 / Slot ③ 动态
...
#
# P1 目标态：动态 Few-shot，每条示例独立存入 store
#   写入（初始化脚本，一次性导入）：
...
```python
# LangGraph AsyncPostgresStore 官方 API（基于当前项目实现）
# 导入路径
...
await runtime.store.aput(
    ...
    "episodic",              # key（Episodic 单条，固定 key）
    episodic_data.model_dump()  # ⚠️ 必须 .model_dump()，不能直接传 Pydantic 对象
)
await runtime.store.aput(
    ("profile", user_id),
    "procedural",            # key（当前实现单条记录，workflows 字典）
    procedural_data.model_dump()
)

# ── 取（Read）────────────────────────────────────────────────────────────
# store.aget(namespace, key) → Item | None，Item.value 是原始 dict
item = await runtime.store.aget(("profile", user_id), "episodic")
if item:
    episodic = UserProfile(**item.value)   # dict → Pydantic 重建

# procedural（当前实现）：
proc_item = await runtime.store.aget(("profile", user_id), "procedural")
procedural = ProceduralMemory(**proc_item.value) if proc_item else ProceduralMemory()

# ── 两种 store 访问方式 ──────────────────────────────────────────────────
# 方式 A：create_agent + AgentMiddleware（推荐，langchain v1 新 API）
#   context_schema：@dataclass 或 TypedDict（当前项目使用 @dataclass）
#   store 访问：runtime.store.aget/asearch（在 abefore_agent 内）
#   wrap_model_call：仅读 request.state["memory_ctx"]，避免每次 LLM 调用重复 IO
#   context 访问：request.runtime.context.user_id（属性风格，@dataclass）
...
class MemoryMiddleware(AgentMiddleware):
    async def abefore_agent(self, state, runtime):
        ...
        item = await runtime.store.aget(("profile", user_id), "episodic")
        # ... 将 item.value 重建后写入 state["memory_ctx"]
        return {"memory_ctx": ...}

    async def awrap_model_call(self, request: ModelRequest, handler):
        memory_ctx = request.state.get("memory_ctx") if request.state else None
        # ... 基于 memory_ctx 注入 messages（request.override(messages=...)）
...
用户画像 Episodic       Slot ⑤        500 Token   Long Memory(DB)      ❌    每次 LLM 调用前      P0
                       （Ephemeral：wrap_model_call → request.override(messages=...) 前置于最后 HumanMessage）
Dynamic Few-shot       Slot ④        800 Token   向量存储(DB)          ❌    每次 LLM 调用前      P1（目标态，当前未实装）
                       （Ephemeral：wrap_model_call → request.override(messages=...) 前置于最后 HumanMessage）
RAG 背景知识            Slot ⑤        2,000 Token 向量存储(DB)          ❌    每次 LLM 调用前      P2（目标态，当前未实装）
                       （Ephemeral：同 Dynamic Few-shot，wrap_model_call → request.override(messages=...)）
Procedural             Slot ⑦        400 Token   Long Memory(DB)      ❌    每次 LLM 调用前      P2
                       （Ephemeral：wrap_model_call → request.override(messages=...)，与 episodic 同链路）
Active Skill 完整内容   Slot ⑨ 历史区  ~1,500 Token 文件系统             ✅    LLM read_file       P1
...
| Memory Injector         | Episodic 动态注入（Ephemeral）                    | `AgentMiddleware.wrap_model_call` → `request.override(messages=...)` 前置于最后 HumanMessage | 🔧 自行开发     | 🔴 P0     |
| Token Budget Manager    | 预算常量定义                                      | `config.py` 手写常量（含 `SLOT_ACTIVE_SKILL` / `SLOT_FEW_SHOT` / `SLOT_PROCEDURAL`）          | 🔧 自行开发     | 🔴 P0     |
| Token Budget Manager    | 超限检测与历史压缩                                   | `SummarizationMiddleware(trigger=..., keep=...)`                                           | ✅ 框架内置      | 🔴 P0     |
| **Agent Skill Manager** | **Skill Registry 初始化（元数据写入 System Prompt）** | **`SkillManager.build_snapshot()` + `build_system_prompt(skill_snapshot=...)`** | **🔧 自行开发** | **🟡 P1** |
| **Agent Skill Manager** | **read_file 工具读取 Skill**                    | **`@tool def read_file(path: str) -> str`，读取 skill.md 返回内容**                             | **🔧 自行开发** | **🟡 P1** |
| **Agent Skill Manager** | **skill 内容进入历史（ToolMessage）**               | **LangGraph 框架自动处理，无需额外开发**                                                            | **✅ 框架内置**  | **🟡 P1** |
| Token Counter           | 精确计数（默认）                                  | `app/utils/token.py::count_tokens`（tiktoken）                                             | 🔧 自行开发     | 🔴 P0     |
| Token Counter           | 近似估算（兜底）                                  | 字符估算法（仅降级场景使用）                                                                  | 🔧 自行开发     | ⚪ P2      |
| Compressor              | 历史压缩触发与执行                                   | `SummarizationMiddleware`（`trigger` + `keep`）                                                | ✅ 框架内置      | 🔴 P0     |
| Memory Injector         | Short Memory 恢复                              | `checkpointer` 自动恢复 `state["messages"]`                                               | ✅ 框架内置      | 🔴 P0     |
| Memory Injector         | Episodic 注入（Long Memory）                    | `AgentMiddleware.wrap_model_call`：读取 `request.state["memory_ctx"].episodic`（来源 `aget(("profile", user_id), "episodic")`） | 🔧 自行开发     | 🔴 P0     |
| Memory Injector         | Procedural 注入（Long Memory）                  | `AgentMiddleware.wrap_model_call`：读取 `request.state["memory_ctx"].procedural`（来源 `aget(("profile", user_id), "procedural")`） | 🔧 自行开发     | ⚪ P2      |
...
| Few-shot Builder        | 动态 Few-shot 检索（目标态，当前未实装）              | `AsyncPostgresStore` + pgvector + `AgentMiddleware.wrap_model_call` 内语义检索注入            | 🔧 自行开发     | 🟡 P1     |
| RAG Retriever           | 向量存储（目标态，当前未实装）                        | `AsyncPostgresStore` + pgvector                                                        | ✅ 框架支持      | ⚪ P2      |
| RAG Retriever           | 检索 + Ephemeral 注入（目标态，当前未实装）             | `AgentMiddleware.wrap_model_call` 内检索后 `request.override(messages=...)` 前置于最后 HumanMessage | 🔧 自行开发     | ⚪ P2      |
...
def build_system_prompt(episodic: UserProfile | None = None) -> str:
    parts = [ROLE_TEMPLATE]
...
class TokenBudget:
    ...
    # Active Skill（概念预算）预估占用（P1）
    # 实际上 skill 内容以 ToolMessage 存在于 Slot ⑨（会话历史），而非独立 Slot
    # 此常量用于预算跟踪：估算 read_file 加载 Skill 后 ToolMessage 的 Token 占用
    # 方便 Token Budget Manager 计算 Slot ⑨ 的剩余弹性空间
    SLOT_ACTIVE_SKILL: int = 0    # 当前默认保持 0（预留字段）

    # Slot ④  Few-shot（P1，默认 0）
    ...
    def slot_history(self) -> int:
        ...
        # P1（Few-shot 开）：24,576 - (2000+0+800+0+500+0+1200) = 20,076 Token
        # P2（全开）：24,576 - (2000+0+800+2000+500+400+1200) = 17,676 Token
```
...
class SkillRegistry:
    ...
    def get_metadata_summary(self) -> str:
        ...
        lines.append("激活方式：调用工具 read_file(path=\"<skill file_path>\")")
        return "\n".join(lines)

    def read(self, skill_id: str) -> str:
        """读取 skill.md 完整内容，供 read_file(path=...) 读取"""
...
def read_file(path: str) -> str:
    """
    读取指定文件内容。
    Skill 场景下，LLM 通过 Skill 元数据中的 file_path 调用 read_file，
    将 SKILL.md 完整内容载入为 ToolMessage，供后续推理使用。
    """
    ...
```
...
Step 1 - LLM Thought：
  "用户询问法律效力，这是法律合规问题，需要读取 legal-search skill 指南"
  LLM Action → read_file(path="skills/legal-search/SKILL.md")

Step 2 - Tool Executor：
  调用 read_file("skills/legal-search/SKILL.md")
...
#### Token Counter（🔴 P0 精确版）

```python
# app/utils/token.py — 🔧 自行开发
import tiktoken
...
# context_schema 支持 @dataclass 或 TypedDict（当前项目使用 @dataclass）
@dataclass
class AgentContext:
    user_id: str
...
#
# 注入策略（基于 LangChain 官方推荐）：
#   Episodic / RAG / Dynamic Few-shot → request.override(messages=...)
#     前置注入最后一条 HumanMessage，模型对末尾消息注意力更高
#   Procedural → request.override(messages=...)
#     与 Episodic 同链路，统一做请求级 Ephemeral 注入
#   两者均 Ephemeral：不写入 state["messages"]，历史永远干净
#
class MemoryMiddleware(AgentMiddleware):
    ...
    async def abefore_agent(self, state: Any, runtime: Any) -> dict[str, Any] | None:
        ...
        procedural = (
            ...
            if procedural_data else ProceduralMemory()
        )
        return {
            "memory_ctx": MemoryContext(episodic=episodic, procedural=procedural),
            "memory_ctx_baseline": episodic.model_copy(deep=True),
        }
    ...
    async def awrap_model_call(
        ...
        # Episodic + Procedural：统一由 processors 组装为注入文本
        ...
        return await handler(request.override(messages=messages))  # Ephemeral
...
agent_with_memory = create_agent(
    ...
    context_schema=AgentContext,           # @dataclass，调用时通过 context= 传入
)
...
不用自己写的（✅ 框架内置）：
  ...
  ToolMessage 进入历史            → LangGraph graph execution 自动（含 read_file 读取 Skill）
...
必须自己写的（🔧 自行开发）：
  ...
  MemoryMiddleware               → wrap_model_call 读 memory_ctx + override messages
  SummarizationMiddleware 参数   → trigger / keep / model 的工程配置
  Skill 元数据拼装               → SkillManager.build_snapshot + build_system_prompt
  Token 精确计数                 → `count_tokens`（tiktoken）
