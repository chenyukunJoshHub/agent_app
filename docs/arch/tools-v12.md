...
─── 阶段三：管理层权限决策 ──────────────────────────────────────────────────
  ...
  │       → 用户确认后整批恢复执行
  │     严格边界：
  │       只要坚持复用 LangChain / LangGraph 官方 ToolNode，而不自定义拆分执行图，
  │       同一批 tool_calls 中一旦同时出现 allow 和 ask，
  │       middleware 阶段就会整体暂停，待 resume 后再一起进入官方 tools executor
...
backoff（重试退避策略）: BackoffConfig | None
  · 重试之间的等待策略，由执行层在重试前读取
  · None = 不重试（max_retries=0 时）
  · 结构：{ "strategy": "fixed"|"exponential", "base_seconds": number }
...
audit_tags（审计标签）: list[str]
  ...
  · 安全约束：SSE trace 中的工具参数需要做脱敏（如邮箱打码、body/token/api_key 置为 [REDACTED]）
```
...
ToolManager
  ...
  方法：
    get_meta(工具名)       → 返回 ToolMeta 副本 | None（防止调用方污染内部注册表）
...
PolicyEngine
  决策依据（优先级从高到低）：
    1. session（会话）级用户授权记录
       ...
       当前实现边界：该授权记录仅存于当前后端进程内的 PolicyEngine 运行时内存，
       不写入 PostgreSQL / checkpointer / Long Memory Store
       因此仅在“同一 session + 同一后端进程存活期”内生效
       后端重启后，session grant 会丢失，策略回退到 effect_class 默认规则
...
Tool ↔ HIL 模块
  ...
  session grant 与 HIL 断点不是同一种状态：
    · HIL 断点 = checkpointer 持久化，跨进程 / 跨重启可恢复
    · session grant = PolicyEngine 运行时内存，当前实现下跨重启不可恢复
  因此 backend 重启后：
    · 已暂停的 HIL interrupt 仍可从 checkpoint 恢复
    · 「本会话已放行」列表会清空，后续同工具调用重新按默认策略决策
  严格边界：在“不改官方 ToolNode”的前提下，HIL 发生在工具真正执行前
  因此同一批 tool_calls 里只要混入 ask 调用，整批会先暂停，再在 resume 后统一进入官方并发执行器
```
...
├── _url_safety.py     # 🔧 SSRF / URL scheme 校验辅助
├── search.py          # effect_class=read，web_search
├── fetch.py           # effect_class=read，fetch_url
├── csv_analyze.py     # effect_class=read
├── file.py            # effect_class=read，read_file
...
说明：
- 当前实现只对 `activate_skill` 做了 `readonly/` 子目录拆分，其余工具仍位于 `tools/` 根目录。
- `readonly / write / orchestration` 的完整目录分层仍是可选重构目标；本阶段文档以当前已落地结构为准。
...
class IdempotencyStore:
    ...
    当前实现为进程内存存根：backend 重启或多 worker 间不共享。
