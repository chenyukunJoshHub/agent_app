...
> 它定义 LLM 在 skill 使用过程中必须遵守的四个约定。
...
System Prompt 中 Skill Protocol 的写入位置：
  Slot ①  System Prompt
          = 角色定义
          + Skill Protocol（四个约定，约 200 Token，常驻）   ← 新增
...
────────────    ────────────────────────    ─────────────────────────────
比喻            餐厅"用餐规则"              餐厅"今日菜单"
内容            四个行为约定（文字）         skill 列表（XML，含 file_path）
...
触发决策逻辑：
  ...
  · 同一 mutex_group 只激活 priority 最高的一个
...
③ 组装 Context Window（工作记忆）
      ...
      │ Slot ①  System Prompt                                     │
      │         = 角色定义                                         │
      │         + Skill Protocol（四个约定，约 200 Token，常驻）    │
...
互斥规则（同一 mutex_group 内）：
  ...
  │         但 legal-search 是当前活跃 skill                  │
  │         Token 超限时，压缩策略优先清理非活跃的同组 skill    │
  └──────────────────────────────────────────────────────────┘
...
其他上限：
  maxSkillsInPrompt   = 150 个
  maxSkillFileBytes   = 256,000 bytes（单个 SKILL.md 大小上限）
```
...
根因：
  ...
  · P0（InMemorySaver）：数据存内存，进程重启即丢失
    → skill ToolMessage 只在当前进程运行期间存在
    → 重启即重置，实际无持久化问题

  · P1（PostgresSaver）+ 永久 thread_id（用户对话历史设计）：
...
应对策略（有明确来源的方案）：
  ...
  ✅ 源头控制体积（OpenClaw 源码验证）：
     · 单个 SKILL.md 体积上限：maxSkillFileBytes = 256,000 bytes
  ...
  🔧 P1 可做（自研，无行业参考）：
     · Context Window 压缩时，同 mutex_group 的非活跃 skill ToolMessage 优先清理
...
根因（精确表述）：
  ...
  · 跨会话是否成立，取决于 thread_id 策略：
      ...
      用户共用持久化 thread_id → P1（PostgresSaver）起，skill 内容从数据库恢复长期存在
  · P0（InMemorySaver）：进程重启即重置，实际无持久化问题
...
──────────────────────────────────────────────────────────────────────────────
Core Tools 注册（含 read_file）        @tool 装饰器（langchain.tools）          ✅ 框架内置
  read_file 等工具函数                  各 @tool 函数体                         🔧 自行开发

Skill Protocol + SkillSnapshot 注入   SystemMessage（Agent 初始化时写入）       ⚡ 胶水代码
  SkillManager.build_snapshot()        扫描目录、解析 frontmatter、生成 prompt  🔧 自行开发

工具注册到 Agent                       model.bind_tools([...tools])             ✅ 框架内置

read_file ToolMessage 进历史           LangGraph ToolMessage（自动）             ✅ 全自动
  read_file 读取 SKILL.md 内容         → 框架自动封装为 ToolMessage              ✅ 全自动

Skill ToolMessage 持久化               PostgresSaver（checkpointer）             ✅ 全自动

mutex_group 互斥检测                   SkillManager 检查 state["messages"]       🔧 自行开发
```
...
from langchain.tools import tool  # ✅ langchain.tools
...
短期记忆（Skill ToolMessage 持久化）：
  P0  → InMemorySaver（langgraph.checkpoint.memory）✅
  P1  → PostgresSaver.from_conn_string(DB_URI)       ✅
...
🔧 自行开发：
  ...
  Skill Protocol 文本       四个约定写入 System Prompt 的自然语言文本
  read_file 函数体          文件读取逻辑（含 ~ 展开、大小校验）
  字符预算三级降级           完整 → 紧凑 → 二分截断
  mutex_group 检测           检查 state["messages"] 中同组 ToolMessage
```
...
```
🔴 P0（当前阶段）：
  · 实现 read_file @tool（含 ~ 路径展开）
  · 实现 SkillManager：
      scan()           扫描 skills/，解析 frontmatter（name / description / tools / file_path）
      build_snapshot() 生成 SkillSnapshot（完整格式，不含降级逻辑）
  · 编写 Skill Protocol 文本（四个约定）
  · 将 Protocol + SkillSnapshot.prompt 注入 System Prompt Slot ①
  · 注册 Core Tools 到 Agent（含 read_file）
  · 验收：LLM 能按 Protocol 调用 read_file 激活 skill，instructions + examples 在下一 turn 可见

🟡 P1：
  · 字符预算三级降级（完整 → 紧凑 → 二分截断）
  · mutex_group 互斥检测（同组旧 skill 标记为非活跃，Token 超限时优先清理）
  · invocation_policy（disable_model_invocation 过滤）
  · tools 依赖检查（skill 所需 core tool 不存在时给出警告）
  · maxSkillFileBytes 大文件跳过保护
