...
**先实现，后自评审；未经过 reviewer 视角自评审的任务，一律不得判定为完成。**

### 执行要求

任何任务在标记“完成”前，必须额外做一轮独立自评审。  
“功能跑通”只代表实现完成，不代表任务完成。只有当以下维度都检查通过，才允许交付：
...
- 边界条件是否覆盖
- 失败路径是否可恢复、可回滚、可重试
- 跨层契约是否一致（后端行为、SSE payload、前端展示、用户实际感知一致）
- 测试覆盖是否包含新增逻辑与关键异常路径

### 自评审视角

自评审时，必须切换到 code review / reviewer 视角，而不是继续沿用实现者视角：

- 不再问“代码能不能跑”
- 要问“哪里会出错、误导用户、破坏状态一致性、留下不可恢复坏状态”
- 优先检查状态机、权限流、审批流、幂等、恢复、前后端交互、批量/混合场景

### 禁止行为

- ❌ 只因主流程跑通、测试变绿就宣布完成
- ❌ 未检查失败路径、回滚路径、边界条件就交付
- ❌ 未核对前端展示与后端真实行为是否一致就交付
- ❌ 把“实现完成”当成“任务完成”
